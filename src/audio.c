...
bool decode_and_mix(audio_manager *am) { /*{{{*/
	...
	for (kab_iter kit=SLL_ISTART(&am->play.buffer_list); !kab_iisend(&kit); kab_inext(&kit)) {
		...
		if ((!kab->prebuffering) || (ap_lsize(&kab->buffer) > am->cfg->prebuffer_amount)) {
			...
			if (ap != NULL) {
				decoded = true;
				int n = opus_decode(kab->decoder, ap->audio.opus.data, ap->audio.opus.len, am->play.pcmbuf, am->cfg->packetlen_samples, 0);
...
bool interpret_contents(audio_packet *ap, bool local) { /*{{{*/
	...
	if (ap->type == PING) {
		...
		ap->audio.target = dptr[0] & 0x1f;
		left -= 1; dptr += 1;

		size_t delta = 0;

		if (local) {
			ap->audio.sid = 0x2320c0ffee15dea7;
		}
		else {
			delta = varint_decode(&ap->audio.sid, dptr, left);
			left -= delta; dptr += delta;
		}

		delta = varint_decode(&ap->audio.seq, dptr, left);
		left -= delta; dptr += delta;
...
audio_packet *build_opus_packet_from_captured_data(audio_manager *am) { /*{{{*/
	...
	audio_packet *ap = get_audio_packet(am, AP_OUT_STATICS_UB + enc_len_est*3/2);
	...
	ap->type = OPUS;
	ap->audio.target = am->cap.target;
	ap->audio.sid    = am->cap.sid;
	ap->audio.seq    = am->cap.seq++; // ++ !
	ap->audio.opus.islast = am->cap.islast;

	uint16_t m = 2+4;

	metadata[m] = ap->type | (ap->audio.target & 0x1f);
	++m;

	m += varint_encode(&metadata[m], 9, ap->audio.seq);

	uint16_t n = ap->audio.opus.len = opus_encode(am->cap.encoder, am->cap.pcmbuf, am->cfg->packetlen_samples, ap->data+AP_OUT_STATICS_UB, (ap->dsz-AP_OUT_STATICS_UB)&0x1fff);

	m += varint_encode(&metadata[m], 9, (ap->audio.opus.islast ? 0x2000 : 0x0000) | n);
